...
from query_tables.db import SQLiteQuery, AsyncSQLiteQuery
...
class TestTables(BaseTest):
    ...
    def setUpClass(cls):
        ...
        async_sqlite = AsyncSQLiteQuery(tests_dir / 'test_tables.db')
        async def get_async_sqlite():
            table = TablesAsync(async_sqlite)
            await table.init()
            return table
        cls.async_sqlite_tables = asyncio.run(get_async_sqlite())
        async def get_async_sqlite_cache():
            table = TablesAsync(async_sqlite, non_expired=True)
            await table.init()
            return table
        cls.async_sqlite_cache_tables = asyncio.run(get_async_sqlite_cache())
    ...
    def test_case_4(self):
        ...
        logger.info("-- Без кеша.")
        asyncio.run(
            self._async_common_query(self.async_sqlite_tables)
        )
        self.async_sqlite_tables.clear_cache()
        logger.info("-- С постоянным кешем.")
        asyncio.run(
            self._async_common_query(self.async_sqlite_cache_tables)
        )
        ...
        logger.info("-- С постоянным кешем.")
        asyncio.run(
            self._async_cache_query(self.async_sqlite_cache_tables)
        )
        self.async_sqlite_cache_tables.clear_cache()
        
        logger.info("- Изменение данных в БД.")
        logger.info("-- С кешем.")
        asyncio.run(self._async_change_db_query(self.async_sqlite_cache_tables))
        logger.info("-- Без кеша.")
        asyncio.run(self._async_change_db_query(self.async_sqlite_tables))
