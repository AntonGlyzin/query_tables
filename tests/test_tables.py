...
class TestTables(BaseTest):
    ...
    async def _async_cache_query(self, c_tables: TablesAsync):
        ...
        query = c_tables['person'].filter(id=2)
        await query.get()
        tmp=await query.cache.get()
        logger.debug(tmp)
        self.assertEqual(len(tmp), 1)
        self.assertEqual(tmp[0]['person.name'], 'Anton 2')
        await c_tables.clear_cache()
        ...
        tmp=await query.cache.get()
        logger.debug(tmp)
        self.assertEqual(tmp[-1]['person.name'], 'Tony 1')
        self.assertEqual(tmp[-1]['person.age'], 32)
        ...
        await query2.get()
        await query3.get()
        tmp=await query1.cache.get()
        self.assertTrue(tmp)
        tmp=await query2.cache.get()
        self.assertTrue(tmp)
        tmp=await query3.cache.get()
        self.assertTrue(tmp)
